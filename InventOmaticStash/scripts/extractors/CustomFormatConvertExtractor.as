   ...
   public class CustomFormatConvertExtractor extends BaseItemExtractor
   {
      ...
      override public function setInventory(param1:MovieClip) : void
      {
         ...
         try
         {
            ...
            Logger.get().info("Starting items data file conversion from json to csv format!");
            ShowHUDMessage("Starting items data file conversion from json to csv format!!",true);
            Logger.get().info("Loading json file: " + (this._extractConfig.customFormat.itemsmodJSONFile || "../itemsmod.ini"));
            setTimeout(loadItemsmodFile,250);
         }
         catch(e:*)
         {
            Logger.get().error("Error in " + modName + " setInventory " + e);
         }
      }
      
      public function loadItemsmodFile() : void
      {
         ...
         try
         {
            ...
            {
               var jsonData:Object;
               var charInventories:Object;
               var decoder:JsonDecoderAsync;
               try
               {
                  Logger.get().info("Inventory json file loaded!");
                  decoder = new JsonDecoderAsync(loader.data,false);
                  if(!decoder.process())
                  {
                     Logger.get().error("JSONDecoder error: " + decoder.result);
                     ShowHUDMessage("JSONDecoder error: " + decoder.result,true);
                     return;
                  }
                  jsonData = decoder.result;
                  if(jsonData.characterInventories == null)
                  {
                     Logger.get().error("Character inventories not found in json file!");
                     ShowHUDMessage("Character inventories not found in json file!",true);
                     return;
                  }
                  charInventories = null;
                  for(char in jsonData.characterInventories)
                  {
                     charInventories = jsonData.characterInventories[char];
                  }
                  if(charInventories == null)
                  {
                     Logger.get().error("Character inventories in json file is empty!");
                     ShowHUDMessage("Character inventories in json file is empty!",true);
                     return;
                  }
                  Logger.get().info("Parsing json file!");
                  FILE_ACCOUNT_DATA = charInventories.AccountInfoData;
                  FILE_CHARACTER_DATA = charInventories.CharacterInfoData;
                  if(FILE_ACCOUNT_DATA == null || FILE_CHARACTER_DATA == null)
                  {
                     Logger.get().error("Character data not found in json file!");
                     ShowHUDMessage("Character data not found in json file!",true);
                     return;
                  }
                  playerInventory = [].concat(charInventories.playerInventory);
                  stashInventory = [].concat(charInventories.stashInventory);
                  Logger.get().info("Extracting data from json file!");
                  setTimeout(extractItems,250);
               }
               catch(e:Error)
               {
                  Logger.get().error("Error parsing items json file: " + e);
                  ShowHUDMessage("Error parsing items json file: " + e,true);
               }
            };
            url = new URLRequest(this._extractConfig.customFormat.itemsmodJSONFile || "../itemsmod.ini");
         ...
         {
            Logger.get().error("Error loading json file: " + e);
            ShowHUDMessage("Error loading json file: " + e,true);
         }
      }
      
      override public function buildOutputObject() : Object
      {
         if(this._extractConfig.customFormat.format.toLowerCase() == "json_to_csv")
         {
      ...
      {
         var characterInfoData:Object = FILE_CHARACTER_DATA;
         var accountInfoData:Object = FILE_ACCOUNT_DATA;
